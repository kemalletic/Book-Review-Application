...
const Home: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    fetch('/api/books?size=3&sortBy=rating&sortDir=DESC', {
      ...
      .catch(err => {
        setError(err.message || 'Error fetching featured books');
        setLoading(false);
      });
  }, []);
              ...
              <Grid item key={book.id} xs={12} sm={6} md={4}>
                ...
                >
                  <Box sx={{ position: 'relative', paddingTop: '150%' }}>
                    <CardMedia
                      component="img"
                      image={book.cover && book.cover !== 'string' ? book.cover : '/default-cover.png'}
                      alt={book.title}
                      sx={{
                        position: 'absolute',
                        top: 0,
